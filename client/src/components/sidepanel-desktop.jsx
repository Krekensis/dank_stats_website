...
const crosshairPlugin = {
    ...
    afterDraw: (chart) => {
        if (chart.tooltip?._active && chart.tooltip._active.length) {
            ...
            ctx.lineWidth = 1;
            ctx.strokeStyle = typeof window !== 'undefined' ? getComputedStyle(document.documentElement).getPropertyValue('--theme-border2').trim() || 'rgba(164, 187, 176, 0.2)' : 'rgba(164, 187, 176, 0.2)';
            ctx.setLineDash([4, 4]);
            ctx.stroke();
            ctx.restore();
        }
...
const SidePanelDesktop = ({ item, prefetchItemIds = [] }) => {
    ...
    return (
        <div className="w-[410px] bg-bg4 border-0 rounded-md p-5 h-full overflow-y-auto flex-shrink-0">
            {item ? (
                <>
                    {/* Header */}
                    <div className="flex items-center gap-4 mb-4 pb-4 border-b-2 border-bg10">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-textMuted font-mono">Item details &amp; stats</p>
                        </div>
                    </div>
                    ...
                    {/* Value Trend */}
                    <div className='mb-4 border-b-2 border-bg10'>
                        <div className="mb-6">
                            <div className="mb-4 flex items-center justify-between">
                                <p className="text-base text-textMuted font-mono">Value Trend</p>
                        ...
                        <div className="grid grid-cols-2 gap-4 mb-4">
                            <div className="bg-bg3 rounded-md py-3 px-2">
                                <p className="text-xs font-mono text-textMuted mb-1">Current Value</p>
                                <p className="text-base font-mono">⏣ {commas(current ?? '–')}</p>
                            </div>
                            <div className="bg-bg3 rounded-md py-3 px-2">
                                <p className="text-xs font-mono text-textMuted mb-1">Oldest Value</p>
                                <p className="text-base font-mono">⏣ {commas(oldest ?? '–')}</p>
                            </div>
                            <div className="bg-bg3 rounded-md py-3 px-2">
                                <p className="text-xs font-mono text-textMuted mb-1">Minimum</p>
                                <p className="text-base font-mono">⏣ {commas(min)}</p>
                            </div>
                            <div className="bg-bg3 rounded-md py-3 px-2">
                                <p className="text-xs font-mono text-textMuted mb-1">Maximum</p>
