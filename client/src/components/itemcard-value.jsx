...
const ItemCard = ({ item, startDate, endDate }) => {
    ...
    const getColorClass = (val) => {
        if (val === "N/A" || !val) return "text-textMuted";
        const num = parseFloat(val);
        return num > 0 ? "text-primary" : num < 0 ? "text-[#ff6b6b]" : "text-textMuted";
    }
        ...
        <div key={item.name} className="bg-bg4 border border-transparent rounded-xl p-4 shadow-lg font-mono flex flex-col w-full sm:w-[235px] sm:shrink-0">
            ...
            <div className="text-[12px] text-textMuted space-y-4 w-full flex-grow">
                <div className="bg-bg1 p-3 rounded-lg border border-transparent">
                    <div className="text-primary font-bold text-[12px] uppercase tracking-wider mb-2">Historical Value</div>
                ...
                <div className="bg-bg1 p-3 rounded-lg border border-transparent">
                    <div className="text-primary font-bold text-[12px] uppercase tracking-wider mb-2">Value Over Range</div>
                    <div className="space-y-1.5">
