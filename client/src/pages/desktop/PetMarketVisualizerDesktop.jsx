...
import PetMultiSelect from "../../components/petmultiselect";

import { useMongoPetData } from "../../hooks/useMongoPetData";
import petMarketCache from "../../hooks/petMarketCache";
...
const PetMarketVisualizerDesktop = () => {
  ...
  const { data: petData, loading: itemsLoading } = useMongoPetData();
  ...
  useEffect(() => {
    if (itemsLoading || !petData) return;
  ...
  const fetchMarketData = useCallback(async (itemID, itemName, start, end, onProgress) => {
    ...
    const missing = petMarketCache.getMissingSegments(itemID, start, end);

    if (missing.type === 'none') {
      console.log(`Cache hit for ${itemName} — no fetch needed`);
      if (onProgress) onProgress(100, 100);
      return petMarketCache.getData(itemID, start, end);
    }
    ...
    <div className="min-h-screen bg-[#070e0c] text-white p-6">
      ...
      {itemsLoading ? (
        ...
        <div className="max-w-6xl mx-auto mt-20 mb-[19px] flex flex-row justify-center items-center space-x-4">
          <PetMultiSelect items={items} selectedItems={selectedItems} setSelectedItems={setSelectedItems} maxSelected={MAX_SELECTED_ITEMS} />
...
export default PetMarketVisualizerDesktop;