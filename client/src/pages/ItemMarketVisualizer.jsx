...
const ItemMarketVisualizer = () => {
  ...
  const fetchMarketData = useCallback(async (itemName, startDate, endDate, onProgress) => {
    ...
    try {
      // 1. Count
      const countRes = await fetch(`https://dankstats.onrender.com/api/marketlogs?${new URLSearchParams({
        ...baseParams,
        countOnly: "true"
      })}`);
      ...
      if (count <= limit) {
        ...
        const res = await fetch(`https://dankstats.onrender.com/api/marketlogs?${new URLSearchParams({
          ...baseParams,
          skip: "0",
      ...
      for (let i = 0; i < pages; i++) {
        ...
        const url = `https://dankstats.onrender.com/api/marketlogs?${params}`;
